...
class CamionTest extends TestCase
{
    ...
    {
        $user = User::first();
        $response = $this->actingAs($user)->get('/api/v3/contenidos/45000000');
    ...
    {
        $user = User::first();
        $response = $this->actingAs($user)->get('/api/v3/contenidos/{}');
    ...
    {
        $user = User::first();
        $response = $this->actingAs($user)->get('/api/v3/contenidos/99999999');
        $response->assertStatus(200);
